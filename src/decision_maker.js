...
function updateKPIs(asteroidData) {
  const hazardousAsteroids = asteroidData.filter(
    (a) => a.is_potentially_hazardous_asteroid === 1
  );
  document.getElementById("total-hazards").textContent =
    hazardousAsteroids.length.toLocaleString();

  const sortedByDate = [...hazardousAsteroids].sort(
  ...
  document.getElementById("next-threat-date").textContent = `Approach: ${
    nextThreat.close_approach_date.split("T")[0]
  }`;

  const sortedByScore = [...asteroidData].sort(
    (a, b) => b.hazard_score - a.hazard_score
  );
  const topThreat = sortedByScore[0];
  document.getElementById("top-threat-name").textContent = topThreat.name;
  document.getElementById(
    "top-threat-score"
  ).textContent = `Hazard Score: ${topThreat.hazard_score.toFixed(2)}`;
}
...
function createCharts(asteroidData) {
  ...
  const top5Threats = [...hazardousAsteroids]
  ...
  // --- Chart 3: Timeline ---
  const upcomingThreats = [...hazardousAsteroids]
    .sort(
      (a, b) =>
