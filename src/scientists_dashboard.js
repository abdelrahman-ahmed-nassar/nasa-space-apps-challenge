document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyFilters() {
        ...
        const minDiameter = parseFloat(document.getElementById('min-diameter').value);
        const maxDiameter = parseFloat(document.getElementById('max-diameter').value);
        const minVelocity = parseFloat(document.getElementById('min-velocity').value);
        const maxVelocity = parseFloat(document.getElementById('max-velocity').value);
        const startDate = document.getElementById('start-date').value;
        const endDate = document.getElementById('end-date').value;

        return allAsteroids.filter(a => {
            // Hazard Filter
            if (hazardFilter === 'hazardous' && a.is_potentially_hazardous_asteroid !== 1) return false;
            if (hazardFilter === 'non-hazardous' && a.is_potentially_hazardous_asteroid === 1) return false;
    ...
    function updateCharts(data) {
        ...
        charts.correlation = new Chart(document.getElementById('correlation-chart'), {
            ...
            options: { scales: { x: { title: { display: true, text: 'Diameter (km)' } }, y: { title: { display: true, text: 'Velocity (km/s)' } } } }
        });
        ...
        charts.distribution = new Chart(document.getElementById('distribution-chart'), {
            type: 'bar',
            data: {
                labels: bins.map((bin, i) => i < bins.length -1 ? `${bin}-${bins[i+1]}` : `${bin}+`),
                datasets: [{
